...
public class DriveCommand extends Command {
  ...
  public void execute() {
    ...
    xSpeed = xLimiter.calculate( // Apply the deadband for the x-axis, then rate limit the value. Finally,
    ...
    ySpeed = yLimiter.calculate( // Apply the deadband for the y-axis, then rate limit the value. Finally,
                                 // multiply by the max module speed to convert to m/s.
        MathUtil.applyDeadband(ySupplier.getAsDouble(), deadbands[1]) * KinematicsConstants.MAX_MODULE_SPEED);
    zSpeed = zLimiter.calculate( // Apply the deadband for the z-axis, then rate limit the value. Finally,
                                 // multiply by the max module speed to convert to m/s.
        MathUtil.applyDeadband(zSupplier.getAsDouble(), deadbands[2]) * KinematicsConstants.MAX_MODULE_SPEED);

    ChassisSpeeds driveSpeeds = ChassisSpeeds.fromFieldRelativeSpeeds(xSpeed, ySpeed, zSpeed,
        driveSubsystem.getHeading()); // Convert the speeds from the joysticks to a field-relative ChassisSpeeds,
